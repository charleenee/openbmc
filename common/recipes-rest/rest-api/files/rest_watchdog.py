        ...
        try:
            with open(os.path.join("/proc", pid, "cmdline"), "rb") as cf:
                ...
                if cmdline == killcmd:
                    syslog.syslog(
                        syslog.LOG_ERR,
                        "Killing leftover rest "
                        "process: pid={} ({})...".format(
                            pid, cmdline.replace("\x00", " ")
                        ),
                    )
...
def main():
    ...
    syslog.syslog(
        syslog.LOG_INFO,
        "REST API watchdog checking %s, every %d seconds" % (checkurl, interval),
    )
    while True:
        global graceleft
        time.sleep(interval)
        ...
        if status != 0:
            syslog.syslog(
                syslog.LOG_WARNING, "REST API not running (sv status: %d)" % (status,)
            )
            continue
        else:
            try:
